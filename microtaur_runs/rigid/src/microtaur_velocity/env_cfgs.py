"""Microtaur velocity-control environment based on the Olympus walking setup.

The policy controls the two actuated joints in each planar five-bar leg, for a
total of eight motor-position actions. The environment keeps the Olympus ideas
that transfer cleanly to Microtaur: predictive command filtering, action and
observation delay, IK-consistent resets, velocity tracking, and locomotion
regularization. Microtaur has no lateral ab/ad joint, so lateral velocity is
fixed to zero and the command space is forward velocity plus yaw rate.

Resets sample a base pose and reachable foot targets, solve the complete
closed-chain joint state analytically, and write active and passive joints
together. Robot-specific joint limits, actuator properties, reward scales, and
terrain dimensions are defined in Microtaur units rather than copied directly
from Olympus.
"""

from __future__ import annotations

ENV_CFG_REVISION = "2026-09-05-rigid-aligned-baseline-v1"
...
# ROBOTIS XL330-M077-T output-shaft data for all eight leg motors:
...
def _ordered_joint_ids(
  ...
  return torch.tensor(
    [id_by_name[name] for name in names],
    device=device,
    dtype=torch.long,
  )
...
def microtaur_ik_consistent_reset(
  ...
  *,
  ...
  _safe_log(
    env,
    "Metrics/ik_reset/valid_fraction",
    torch.mean(valid_per_leg.float()),
  )
  _safe_log(
    env,
    "Metrics/ik_reset/root_height",
    torch.mean(root_height),
  )
...
FORWARD_TRACKING_SIGMA_M_S = float(
  os.environ.get("MICROTAUR_FORWARD_TRACKING_SIGMA", "0.04")
)
...
# Interpret uprightness directly as chassis tilt angle.
# At 15 degrees tilt, the upright reward is 0.5.
...
ZERO_COMMAND_YAW_RATE_SCALE_RAD_S = 0.15
JOINT_ACCELERATION_SCALE_RAD_S2 = 150.0
AIR_TIME_TARGET_S = 0.15
AIR_TIME_SIGMA_S = 0.045
...
class MicrotaurOlympusWalkAction(JointPositionAction):
  ...
  def __init__(self, cfg: MicrotaurOlympusWalkActionCfg, env):
    ...
    # MJLab v1.2 may return matching joints in model/tree order rather than
    # requested order. Keep policy/kinematics semantics canonical.
    resolved_names = tuple(getattr(self, "_target_names", ()))
    expected_names = tuple(LEG_JOINT_NAMES)
    if not resolved_names:
      resolved_names = expected_names
    if (
      len(resolved_names) != len(expected_names)
...
def microtaur_scaled_walk_reward(
  ...
  joint_pos = term.resolved_to_canonical(
    robot.data.joint_pos[:, term._target_ids]
  )
  joint_acc = term.resolved_to_canonical(
    robot.data.joint_acc[:, term._target_ids]
  )
  ...
  track_yaw = torch.exp(-torch.square(yaw_error)) * yaw_active.float()
  ...
  gravity_unit = projected_gravity_b / torch.clamp(
  ...
  zero_command_yaw_rate = torch.clamp(
    torch.square(
      root_ang_vel_b[:, 2] / ZERO_COMMAND_YAW_RATE_SCALE_RAD_S
    ),
    max=4.0,
  ) * (~yaw_active).float()
  ...
  for name, value in components.items():
    _safe_log(
      env,
      f"Metrics/microtaur_reward/{name}",
      torch.mean(value),
    )
  for name, value in raw_terms.items():
    _safe_log(
      env,
      f"Metrics/microtaur_raw/{name}",
      torch.mean(value),
    )
  ...
  _safe_log(
    env,
    "Metrics/microtaur/command_forward_m_s",
    torch.mean(command_x),
  )
  _safe_log(
    env,
    "Metrics/microtaur/filter_blend_mean",
    torch.mean(term.filter_blend),
  )
  _safe_log(
    env,
  ...
  _safe_log(
    env,
    "Metrics/microtaur/total_reward_pre_dt",
    torch.mean(total),
  )
...
def _configure_observations(
  ...
  # Add one-to-two policy-step sensor latency to actor observations. The critic
  # keeps current state, and hasattr keeps this compatible with MJLab versions
  # that do not expose inline observation delays.
