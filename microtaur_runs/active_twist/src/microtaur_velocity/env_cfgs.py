"""Active-twist Microtaur velocity-control environment.

The policy controls the eight actuated five-bar leg motors plus the active
axial-twist spine joint. Leg commands keep the existing predictive five-bar
safety filter and one-to-two-step command delay. The spine has its own bounded,
delayed position-action path so the policy can discover when trunk twist helps
forward/yaw velocity tracking without placing the spine inside the leg filter.

Whole-body velocity tracking is evaluated in a virtual midpoint chassis frame:
front/rear trunk world velocities are averaged, expressed in the rear trunk
frame, then rotated by half the measured spine angle about the physical
longitudinal twist axis. Uprightness is evaluated on both physical trunk halves
so equal-and-opposite roll cannot exploit the midpoint frame. Spine use is not
directly rewarded or forced, and there is no locomotion-time q->0 penalty; useful
spine motion must earn its keep through better task tracking/stability.
"""

from __future__ import annotations

ENV_CFG_REVISION = "2026-09-05-active-twist-emergent-v4-mean-bias-fix"
...
# ROBOTIS XL330-M077-T output-shaft data for all nine active motors
# (8 leg motors + 1 axial-twist spine motor).
# supply voltage: (stall torque [N m], no-load speed [rpm], stall current [A])
_XL330_TABLE = {
  3.7: (0.12, 280.0, 1.11),
  5.0: (0.215, 383.0, 1.47),
  6.0: (0.30, 123.0, 1.74),
}
...
# Active axial-twist spine. The active-twist XML uses spine_joint_act with a
# physical hinge range of +/-0.35 rad. Keep the training command range inside
# that hard stop and give the spine its own actuator knobs even when it uses the
# same XL330 hardware as the legs.
ACTIVE_SPINE_JOINT_NAME = "spine_joint_act"
ACTIVE_SPINE_REAR_BODY = "twisting_microtaur_bottom_half_motor_mount"
ACTIVE_SPINE_FRONT_BODY = "twisting_microtaur_top_half"
ACTIVE_BODY_COLLISION_GEOMS = ("rear_body_collision", "front_body_collision")

ACTIVE_SPINE_HARD_LIMIT_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_HARD_LIMIT_RAD", "0.35")
)
ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD", "0.32")
)
ACTIVE_SPINE_LIMIT_FREE_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_LIMIT_FREE_RAD", "0.25")
)
ACTIVE_SPINE_LIMIT_SCALE_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_LIMIT_SCALE_RAD", "0.10")
)
ACTIVE_SPINE_STAND_SCALE_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_STAND_SCALE_RAD", "0.10")
)
ACTIVE_SPINE_POWER_SCALE_W = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_POWER_SCALE_W", "0.25")
)

ACTIVE_SPINE_POSITION_KP = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_KP", str(XL330_POSITION_KP))
)
ACTIVE_SPINE_POSITION_KD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_KD", str(XL330_POSITION_KD))
)
ACTIVE_SPINE_ARMATURE = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_ARMATURE", str(XL330_ARMATURE))
)
ACTIVE_SPINE_FRICTIONLOSS_NM = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_FRICTIONLOSS",
    str(XL330_OUTPUT_FRICTIONLOSS_NM),
  )
)
ACTIVE_SPINE_SAFE_EFFORT_NM = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_SAFE_EFFORT_NM", str(XL330_SAFE_EFFORT_NM))
)
ACTIVE_SPINE_SATURATION_EFFORT_NM = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_SATURATION_EFFORT_NM",
    str(XL330_STALL_TORQUE_NM),
  )
)
ACTIVE_SPINE_VELOCITY_LIMIT_RAD_S = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_VELOCITY_LIMIT_RAD_S",
    str(XL330_VELOCITY_LIMIT_RAD_S),
  )
)

# The child trunk has a neutral -90 deg Y frame rotation and local +Z hinge
# axis, which maps the physical axial twist direction to -X in the rear/root
# trunk frame.
ACTIVE_SPINE_AXIS_REAR = (-1.0, 0.0, 0.0)

ACTIVE_SPINE_FILTER_HORIZON_S = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_FILTER_HORIZON_S", str(5.0 / 60.0))
)
ACTIVE_SPINE_FILTER_VELOCITY_FLOOR_RAD_S = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_FILTER_VELOCITY_FLOOR_RAD_S", "0.75")
)
ACTIVE_SPINE_FILTER_MARGIN_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_FILTER_MARGIN_RAD", "0.04")
)
...
def _make_xml_validated_spec() -> mujoco.MjSpec:
  """Load the active-twist XML and repair RL-critical CAD defaults."""
  spec = get_spec()
  ...
  for joint_name in PASSIVE_JOINT_NAMES:
    joint = spec.joint(joint_name)
    if joint is None:
      raise ValueError(f"Missing passive Microtaur leg joint: {joint_name}")
  ...
  spine_joint = spec.joint(ACTIVE_SPINE_JOINT_NAME)
  if spine_joint is None:
    raise ValueError(
      "Active-twist environment requires joint 'spine_joint_act'."
    )
  spine_joint.limited = mujoco.mjtLimited.mjLIMITED_TRUE
  spine_joint.range[:] = np.asarray(
    (-ACTIVE_SPINE_HARD_LIMIT_RAD, ACTIVE_SPINE_HARD_LIMIT_RAD),
    dtype=np.float64,
  )
  spine_joint.armature = ACTIVE_SPINE_ARMATURE
  spine_joint.frictionloss = ACTIVE_SPINE_FRICTIONLOSS_NM
  # Preserve the XML's small physical hinge damping. The servo's KD below is
  # controller damping and is intentionally modeled separately.

  for site_name in FOOT_SITE_NAMES:
    if spec.site(site_name) is None:
      raise ValueError(f"Missing Microtaur foot site: {site_name}")

  for geom_name in ACTIVE_BODY_COLLISION_GEOMS:
    if spec.geom(geom_name) is None:
      raise ValueError(f"Missing active-twist body collision geom: {geom_name}")

  return spec


def _make_microtaur_robot_cfg() -> EntityCfg:
  leg_motor_cfg = DcMotorActuatorCfg(
  ...
  spine_motor_cfg = DcMotorActuatorCfg(
    target_names_expr=SPINE_JOINT_NAMES,
    stiffness=ACTIVE_SPINE_POSITION_KP,
    damping=ACTIVE_SPINE_POSITION_KD,
    effort_limit=ACTIVE_SPINE_SAFE_EFFORT_NM,
    saturation_effort=ACTIVE_SPINE_SATURATION_EFFORT_NM,
    velocity_limit=ACTIVE_SPINE_VELOCITY_LIMIT_RAD_S,
    armature=ACTIVE_SPINE_ARMATURE,
    frictionloss=ACTIVE_SPINE_FRICTIONLOSS_NM,
  )
  ...
  return EntityCfg(
    ...
    articulation=EntityArticulationInfoCfg(
      actuators=(leg_motor_cfg, spine_motor_cfg),
...
COMMAND_STAGES = (
  {
    ...
    "ang_vel_z": (0.00, 0.00),
    "standing": 0.00,
    # First learn a clean leg-driven forward gait with the axial spine neutral.
    "spine_action_scale_rad": math.radians(0.0),
  },
  {
    "step": 25_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.10, 0.10),
    "standing": 0.05,
    "spine_action_scale_rad": math.radians(5.0),
  },
  {
    "step": 50_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.15, 0.15),
    "standing": 0.05,
    "spine_action_scale_rad": math.radians(9.0),
  },
  {
    "step": 70_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.20, 0.20),
    "standing": 0.10,
    "spine_action_scale_rad": math.radians(13.0),
  },
  {
    "step": 90_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.25, 0.25),
    "standing": 0.10,
    "spine_action_scale_rad": math.radians(17.0),
  },
)
...
def _set_runtime_spine_action_scale(
  env,
  action_name: str,
  action_scale_rad: float,
) -> None:
  term = env.action_manager.get_term(action_name)
  if not isinstance(term, MicrotaurActiveSpinePositionAction):
    raise TypeError(
      f"'{action_name}' must be MicrotaurActiveSpinePositionAction; "
      f"got {type(term).__name__}."
    )
  term.cfg.action_scale_rad = min(
    float(action_scale_rad),
    ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD,
  )
...
def microtaur_command_curriculum(
  ...
  _set_runtime_spine_action_scale(
    env,
    spine_action_name,
    float(stage["spine_action_scale_rad"]),
  )
  ...
  return {
    ...
    "spine_action_scale_rad": as_tensor(stage["spine_action_scale_rad"]),
  }
...
def _ordered_joint_ids(
  ...
  """Resolve joint IDs in the exact logical order requested by the caller.

  MJLab v1.2 may return matching joints in MuJoCo model/tree order even when
  preserve_order=True.  Active-spine variants place the front-leg subtree
  before the rear-leg subtree, so model order can be (leg3, leg4, leg1, leg2)
  even though Microtaur kinematics and policy semantics use (leg1..leg4).
  Reorder the returned IDs explicitly by name instead of relying on backend
  ordering.
  """
  ...
  if len(ids) != len(names) or set(resolved) != set(names):
    raise ValueError(
      "Microtaur joint resolution mismatch. "
      f"Expected exactly {names}, got {resolved}."
    )

  id_by_name = {name: int(joint_id) for name, joint_id in zip(resolved, ids)}
  ordered_ids = [id_by_name[name] for name in names]
  return torch.tensor(ordered_ids, device=device, dtype=torch.long)
...
def microtaur_ik_consistent_reset(
  ...
  *,
  ...
  # The leg IK is defined at zero axial twist. Reset the active spine exactly
  # neutral so front/rear foot geometry remains consistent with the solved leg
  # state; recovery randomization can be added later once the base gait is
  # reliable.
  spine_ids = _ordered_joint_ids(robot, tuple(SPINE_JOINT_NAMES), device)
  spine_q = torch.zeros((count, 1), device=device, dtype=dtype)
  spine_qdot = torch.zeros_like(spine_q)
  robot.write_joint_state_to_sim(
    spine_q,
    spine_qdot,
    joint_ids=spine_ids,
...
MICROTAUR_REWARD_SCALES = {
  ...
  "upright": 0.75,
  "feet_air_time": 0.35,
  "moving_contact_pattern": 0.25,
  "standing_pose": 0.50,
  "lateral_velocity": -0.15,
  "vertical_velocity": -0.10,
  "roll_pitch_rate": -0.10,
  "normalized_torque": -0.03,
  "normalized_acceleration": -0.025,
  "action_rate": -0.07,
  "undesired_contacts": -2.00,
  "filter_correction": -0.10,
  "joint_limit_proximity": -0.05,

  # Emergent spine-use philosophy, matching active pitch: there is no positive
  # reward for twisting and no q->0 penalty during locomotion. Useful axial
  # twist must improve locomotion enough to justify its energy/limit costs.
  "spine_limit_proximity": -0.10,
  "spine_power": -0.02,
  "standing_spine_neutral": -0.05,

  # Penalize only the SLOW/DC spine offset during straight locomotion.
  # Fast zero-mean oscillatory twist remains free to emerge.
  "spine_mean_bias": -0.05,
}
...
# For the axial-twist morphology, uprightness is evaluated on BOTH physical
# trunk halves rather than only the virtual midpoint. This closes the
# equal-and-opposite-roll loophole without directly penalizing spine angle.
...
AIR_TIME_TARGET_S = 0.15 #was 0.10
AIR_TIME_SIGMA_S = 0.045 #was 0.06

# Slow/DC axial-spine bias detector.
# alpha=0.02 gives roughly a ~1 s low-pass timescale at the current 50-60 Hz
# control rate: gait-cycle oscillation largely averages out, while a sustained
# one-sided offset remains visible to the reward.
SPINE_MEAN_FILTER_ALPHA = float(
  os.environ.get("MICROTAUR_SPINE_MEAN_FILTER_ALPHA", "0.02")
)
SPINE_MEAN_BIAS_SCALE_RAD = float(
  os.environ.get(
    "MICROTAUR_SPINE_MEAN_BIAS_SCALE_RAD",
    str(math.radians(6.0)),
  )
)
if not 0.0 < SPINE_MEAN_FILTER_ALPHA <= 1.0:
  raise ValueError("MICROTAUR_SPINE_MEAN_FILTER_ALPHA must be in (0, 1].")
...
def _safe_log(env, name: str, value: torch.Tensor) -> None:
  ...
  if isinstance(extras, dict):
    ...
    if isinstance(log, dict):
      log[name] = value
# CPG-RL: four leg oscillators instead of eight direct motor actions
CPG_ENABLED = os.environ.get("MICROTAUR_CPG", "0").lower() not in {"0", "false", "no", ""}
...
class MicrotaurOlympusWalkActionCfg(JointPositionActionCfg):
  ...
  def build(self, env):
    # CPG-RL swaps in the four-oscillator leg action; the CLI parser rebuilds cfg
    # objects, so the switch lives here rather than on the cfg instance.
    if CPG_ENABLED:
      from microtaur_velocity.cpg_action import build_cpg_action_term

      return build_cpg_action_term(self, env)
...
class MicrotaurOlympusWalkAction(JointPositionAction):
  ...
  def __init__(self, cfg: MicrotaurOlympusWalkActionCfg, env):
    ...
    # MJLab v1.2 can resolve matching joints in MuJoCo tree order instead of
    # requested order.  Keep the policy/kinematics convention canonical
    # (leg1 a/e, leg2 a/e, leg3 a/e, leg4 a/e) and explicitly map to/from the
    ...
    if len(resolved_names) != len(expected_names) or set(resolved_names) != set(expected_names):
      raise ValueError(
        "Resolved motor set does not match Microtaur's eight actuated leg joints. "
        f"Expected {expected_names}, got {resolved_names}."
      )

    resolved_index = {name: idx for idx, name in enumerate(resolved_names)}
    canonical_index = {name: idx for idx, name in enumerate(expected_names)}
    self._canonical_from_resolved = torch.tensor(
      [resolved_index[name] for name in expected_names],
      device=device,
      dtype=torch.long,
    )
    self._resolved_from_canonical = torch.tensor(
      [canonical_index[name] for name in resolved_names],
      device=device,
      dtype=torch.long,
    )
  ...
  def resolved_to_canonical(self, value: torch.Tensor) -> torch.Tensor:
    """Reorder a backend target-ordered tensor into leg1..leg4 order."""
    return value[:, self._canonical_from_resolved]

  def canonical_to_resolved(self, value: torch.Tensor) -> torch.Tensor:
    """Reorder a leg1..leg4 tensor into MJLab's target-joint order."""
  ...
  def process_actions(self, actions: torch.Tensor):
    ...
    # JointPositionAction applies processed targets in self._target_ids order,
    # which may differ from the canonical policy order in active-spine XMLs.
...
@configclass
class MicrotaurActiveSpinePositionActionCfg(JointPositionActionCfg):
  """One-dimensional active axial-twist target with limit filtering and delay."""

  class_type: type = MISSING

  action_scale_rad: float = 0.08
  target_limit_rad: float = ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD
  filter_horizon_s: float = ACTIVE_SPINE_FILTER_HORIZON_S
  velocity_floor_rad_s: float = ACTIVE_SPINE_FILTER_VELOCITY_FLOOR_RAD_S
  limit_margin_rad: float = ACTIVE_SPINE_FILTER_MARGIN_RAD
  min_action_delay_steps: int = OLYMPUS_MIN_ACTION_DELAY_STEPS
  max_action_delay_steps: int = OLYMPUS_MAX_ACTION_DELAY_STEPS
  ...
  def __init__(
    ...
    **kwargs,
    ...
    self.class_type = MicrotaurActiveSpinePositionAction
    self.entity_name = "robot"
    self.actuator_names = tuple(SPINE_JOINT_NAMES)
    self.preserve_order = preserve_order
    self.use_default_offset = use_default_offset
    ...
    self.offset = 0.0 if offset is None else offset
    self.action_scale_rad = float(action_scale_rad)
    self.target_limit_rad = float(target_limit_rad)
    self.filter_horizon_s = float(filter_horizon_s)
    self.velocity_floor_rad_s = float(velocity_floor_rad_s)
  ...
  def build(self, env):
    return MicrotaurActiveSpinePositionAction(self, env)


class MicrotaurActiveSpinePositionAction(JointPositionAction):
  """Delayed, bounded spine-position command independent of the leg filter."""

  cfg: MicrotaurActiveSpinePositionActionCfg

  def __init__(self, cfg: MicrotaurActiveSpinePositionActionCfg, env):
    super().__init__(cfg, env)
    self._walk_env = env
    device = env.device

    resolved_names = tuple(getattr(self, "_target_names", ()))
    expected_names = tuple(SPINE_JOINT_NAMES)
    if resolved_names and resolved_names != expected_names:
      raise ValueError(
        "Resolved spine action joint order mismatch. "
        f"Expected {expected_names}, got {resolved_names}."
      )

    self._requested_targets = torch.zeros((env.num_envs, 1), device=device)
    self._safe_targets = self._requested_targets.clone()
    self._applied_targets = self._requested_targets.clone()
    self._filter_correction = torch.zeros_like(self._requested_targets)
    self._filter_blend = torch.zeros_like(self._requested_targets)
  ...
  def _filter_target(
    ...
    limit = min(
      float(self.cfg.target_limit_rad),
      ACTIVE_SPINE_HARD_LIMIT_RAD - 1e-4,
    )
    lower = -limit
    upper = limit

    margin_lower = position - lower
    margin_upper = upper - position
    outside = (margin_lower < 0.0) | (margin_upper < 0.0)

    velocity_floor = max(float(self.cfg.velocity_floor_rad_s), 1e-6)
    velocity_sign = torch.where(
      velocity >= 0.0, torch.ones_like(velocity), -torch.ones_like(velocity)
    )
    safe_velocity = torch.where(
      torch.abs(velocity) < velocity_floor,
      velocity_sign * velocity_floor,
      velocity,
    )
    ...
    margin = float(self.cfg.limit_margin_rad)
    near_upper = margin_upper < margin
    near_lower = margin_lower < margin
    command_out_upper = requested > position
    command_out_lower = requested < position
    blend = torch.where(near_upper & command_out_upper, torch.ones_like(blend), blend)
    blend = torch.where(near_lower & command_out_lower, torch.ones_like(blend), blend)
    blend = torch.where(near_upper & (requested < position), torch.zeros_like(blend), blend)
    blend = torch.where(near_lower & (requested > position), torch.zeros_like(blend), blend)

    clamped = torch.clamp(requested, min=lower, max=upper)
    filtered = (1.0 - blend) * requested + blend * clamped
    filtered = torch.clamp(filtered, min=lower, max=upper)
    return filtered, blend

  def process_actions(self, actions: torch.Tensor):
    if actions.shape[-1] != 1:
      raise ValueError(
        "MicrotaurActiveSpinePositionAction expects one action; "
    ...
    action_scale = min(
      float(self.cfg.action_scale_rad),
      float(self.cfg.target_limit_rad),
    )
    requested = action_scale * actions
    safe, blend = self._filter_target(position, velocity, requested)
...
MicrotaurActiveSpinePositionActionCfg.class_type = MicrotaurActiveSpinePositionAction


def _get_spine_action_term(
  env,
  action_name: str = "spine_pos",
) -> MicrotaurActiveSpinePositionAction:
  term = env.action_manager.get_term(action_name)
  if not isinstance(term, MicrotaurActiveSpinePositionAction):
    raise TypeError(
      f"Action term '{action_name}' must be MicrotaurActiveSpinePositionAction; "
      f"got {type(term).__name__}."
    )
  return term
...
def _ensure_microtaur_reward_state(
  ...
  env._microtaur_scaled_reward_state = {
    ...
    # Low-pass estimate of spine q used only to detect persistent one-sided
    # twist. This does NOT penalize instantaneous/dynamic spine motion.
    "spine_mean_q": torch.zeros(
      env.num_envs,
      device=device,
    ),
  }
...
def _microtaur_active_spine_indices(env) -> tuple[int, int, int, int]:
  """Return local IDs for spine joint, rear body, front body, spine actuator."""
  cached = getattr(env, "_microtaur_active_spine_indices", None)
  if cached is not None:
    return cached

  robot = env.scene["robot"]
  joint_ids, joint_names = robot.find_joints(
    [ACTIVE_SPINE_JOINT_NAME],
    preserve_order=True,
  )
  if len(joint_ids) != 1 or tuple(joint_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active spine joint could not be resolved exactly once.")

  rear_ids, rear_names = robot.find_bodies(
    [ACTIVE_SPINE_REAR_BODY],
    preserve_order=True,
  )
  front_ids, front_names = robot.find_bodies(
    [ACTIVE_SPINE_FRONT_BODY],
    preserve_order=True,
  )
  if len(rear_ids) != 1 or tuple(rear_names) != (ACTIVE_SPINE_REAR_BODY,):
    raise RuntimeError("Rear active-twist trunk body could not be resolved.")
  if len(front_ids) != 1 or tuple(front_names) != (ACTIVE_SPINE_FRONT_BODY,):
    raise RuntimeError("Front active-twist trunk body could not be resolved.")

  actuator_ids, actuator_names = robot.find_actuators(
    [ACTIVE_SPINE_JOINT_NAME],
    preserve_order=True,
  )
  if len(actuator_ids) != 1 or tuple(actuator_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active spine actuator could not be resolved exactly once.")
  ...
  env._microtaur_active_spine_indices = result
  return result


def _microtaur_foot_site_ids(env) -> torch.Tensor:
  """Return foot-site IDs in canonical leg1..leg4 order."""
  cached = getattr(env, "_microtaur_foot_site_ids_cache", None)
  if cached is not None:
    return cached

  robot = env.scene["robot"]
  ids, resolved = robot.find_sites(list(FOOT_SITE_NAMES), preserve_order=True)
  id_by_name = {name: int(site_id) for name, site_id in zip(resolved, ids)}
  missing = [name for name in FOOT_SITE_NAMES if name not in id_by_name]
  if missing:
    raise RuntimeError(f"Missing Microtaur foot sites for slip reward: {missing}")

  ordered = torch.tensor(
    [id_by_name[name] for name in FOOT_SITE_NAMES],
    device=env.device,
    dtype=torch.long,
  )
  env._microtaur_foot_site_ids_cache = ordered
  return ordered


def microtaur_spine_joint_position(env) -> torch.Tensor:
  """Active axial-twist spine q [rad]."""
  robot = env.scene["robot"]
  spine_joint_id, _, _, _ = _microtaur_active_spine_indices(env)
  return robot.data.joint_pos[:, spine_joint_id : spine_joint_id + 1]


def microtaur_spine_joint_velocity(env) -> torch.Tensor:
  """Active axial-twist spine qdot [rad/s]."""
  robot = env.scene["robot"]
  spine_joint_id, _, _, _ = _microtaur_active_spine_indices(env)
  return robot.data.joint_vel[:, spine_joint_id : spine_joint_id + 1]


def _microtaur_midpoint_frame_state(
  env,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
  """Return midpoint-frame (linear velocity, angular velocity, gravity).

  The active CAD's front MuJoCo body frame is rotated relative to the rear
  frame even at q=0, so raw front/rear body-frame vectors must not be averaged.
  Instead, front/rear *world-frame* link velocities are averaged first. The
  result is expressed in the rear/root frame and then rotated by -q/2 about the
  physical axial-twist direction to obtain the virtual midpoint chassis frame.
  """
  robot = env.scene["robot"]
  spine_joint_id, rear_body_id, front_body_id, _ = _microtaur_active_spine_indices(env)

  rear_lin_w = robot.data.body_link_lin_vel_w[:, rear_body_id]
  front_lin_w = robot.data.body_link_lin_vel_w[:, front_body_id]
  rear_ang_w = robot.data.body_link_ang_vel_w[:, rear_body_id]
  front_ang_w = robot.data.body_link_ang_vel_w[:, front_body_id]

  mean_lin_w = 0.5 * (rear_lin_w + front_lin_w)
  mean_ang_w = 0.5 * (rear_ang_w + front_ang_w)

  rear_quat_w = robot.data.body_link_quat_w[:, rear_body_id]
  mean_lin_rear = quat_apply_inverse(rear_quat_w, mean_lin_w)
  mean_ang_rear = quat_apply_inverse(rear_quat_w, mean_ang_w)

  spine_q = robot.data.joint_pos[:, spine_joint_id]
  axis = torch.tensor(
    ACTIVE_SPINE_AXIS_REAR,
    device=mean_lin_rear.device,
    dtype=mean_lin_rear.dtype,
  )
  half_angle = -0.5 * spine_q

  midpoint_lin_b = _rotate_vector_about_axis(mean_lin_rear, axis, half_angle)
  midpoint_ang_b = _rotate_vector_about_axis(mean_ang_rear, axis, half_angle)
  midpoint_gravity_b = _rotate_vector_about_axis(
    robot.data.projected_gravity_b,
    axis,
    half_angle,
  )
  return midpoint_lin_b, midpoint_ang_b, midpoint_gravity_b


def microtaur_midpoint_lin_vel(env) -> torch.Tensor:
  return _microtaur_midpoint_frame_state(env)[0]


def microtaur_midpoint_ang_vel(env) -> torch.Tensor:
  return _microtaur_midpoint_frame_state(env)[1]


def microtaur_projected_gravity(env) -> torch.Tensor:
  return _microtaur_midpoint_frame_state(env)[2]
...
def microtaur_scaled_walk_reward(
  ...
  spine_action_name: str = "spine_pos",
) -> torch.Tensor:
  """Pitch-style emergent locomotion reward with twist-safe whole-body uprightness."""
  robot = env.scene["robot"]
  leg_term = _get_walk_action_term(env, action_name)
  spine_term = _get_spine_action_term(env, spine_action_name)
  command = env.command_manager.get_command(command_name)
  if command is None:
    raise KeyError(f"Command '{command_name}' was not found.")

  leg_joint_pos = leg_term.resolved_to_canonical(
    robot.data.joint_pos[:, leg_term._target_ids]
  )
  leg_joint_acc = leg_term.resolved_to_canonical(
    robot.data.joint_acc[:, leg_term._target_ids]
  )
  spine_joint_id, _, _, spine_actuator_id = _microtaur_active_spine_indices(env)
  spine_joint_acc = robot.data.joint_acc[
    :, spine_joint_id : spine_joint_id + 1
  ]
  active_joint_acc = torch.cat((leg_joint_acc, spine_joint_acc), dim=1)
  actuator_force = robot.data.actuator_force
  action = env.action_manager.action

  # CPG-RL sends three numbers per leg oscillator instead of one per motor.
  expected_actuator_dim = len(LEG_JOINT_NAMES) + len(SPINE_JOINT_NAMES)
  expected_action_dim = (12 + 1) if CPG_ENABLED else expected_actuator_dim
  if action.shape[1] != expected_action_dim:
    raise RuntimeError(
      f"Active-twist policy must expose {expected_action_dim} actions; "
      f"got {action.shape[1]}."
    )
  if actuator_force.shape[1] != expected_actuator_dim:
    raise RuntimeError(
      f"Active-twist robot must expose {expected_actuator_dim} actuators; "
      f"got {actuator_force.shape[1]}."
    )
  ...
  if torch.any(reset):
    ...
    state["spine_mean_q"][reset] = 0.0

  midpoint_lin_vel_b, midpoint_ang_vel_b, projected_gravity_b = (
    _microtaur_midpoint_frame_state(env)
  )
  root_height = _microtaur_root_height(env, robot)
  spine_q = robot.data.joint_pos[:, spine_joint_id]
  spine_qdot = robot.data.joint_vel[:, spine_joint_id]

  # Track only the slow/DC component of axial twist. A clean oscillatory spine
  # gait centered around zero averages out here, whereas the previously observed
  # ~+11 deg permanent bias remains strongly visible.
  alpha = SPINE_MEAN_FILTER_ALPHA
  state["spine_mean_q"] = (
    (1.0 - alpha) * state["spine_mean_q"]
    + alpha * spine_q
  )

  command_x = command[:, 0]
  command_yaw = command[:, 2]
  actual_x = midpoint_lin_vel_b[:, 0]
  ...
  straight_motion = translation_active & (~yaw_active)

  # Penalize sustained one-sided axial twist ONLY while commanded to walk
  # straight. Turning remains free to shift the mean spine angle if useful.
  spine_mean_bias = torch.clamp(
    torch.square(
      state["spine_mean_q"]
      / max(SPINE_MEAN_BIAS_SCALE_RAD, 1e-6)
    ),
    max=4.0,
  ) * straight_motion.float()
  ...
  forward_progress = torch.clamp(
    ...
    min=0.0,
    max=1.0,
  ) * translation_active.float()

  yaw_error = (
    command_yaw - midpoint_ang_vel_b[:, 2]
  ...
  # -------------------------------------------------------------------------
  # Whole-body uprightness for an AXIAL-TWIST spine
  # -------------------------------------------------------------------------
  # A midpoint-only orientation can be exploited: the rear half can roll +q/2
  # while the front half rolls -q/2, leaving the midpoint apparently upright.
  # Instead of directly penalizing spine q (which would suppress emergent use),
  # reconstruct the two physical trunk orientations around the midpoint and
  # reward BOTH halves for remaining upright. A useful transient twist is still
  # allowed; a sustained splayed posture now loses ordinary upright reward.
  midpoint_gravity_unit = projected_gravity_b / torch.clamp(
    torch.linalg.vector_norm(projected_gravity_b, dim=1, keepdim=True),
    min=1e-6,
  )
  midpoint_tilt_angle = torch.atan2(
    torch.linalg.vector_norm(midpoint_gravity_unit[:, :2], dim=1),
    -midpoint_gravity_unit[:, 2],
  )

  spine_axis_mid = torch.tensor(
    ACTIVE_SPINE_AXIS_REAR,
    device=projected_gravity_b.device,
    dtype=projected_gravity_b.dtype,
  )
  half_spine_q = 0.5 * spine_q
  rear_gravity_mid = _rotate_vector_about_axis(
    projected_gravity_b, spine_axis_mid, +half_spine_q
  )
  front_gravity_mid = _rotate_vector_about_axis(
    projected_gravity_b, spine_axis_mid, -half_spine_q
  )

  def _tilt_and_upright(gravity_b: torch.Tensor):
    gravity_unit = gravity_b / torch.clamp(
      torch.linalg.vector_norm(gravity_b, dim=1, keepdim=True),
      min=1e-6,
    )
    tilt = torch.atan2(
      torch.linalg.vector_norm(gravity_unit[:, :2], dim=1),
      -gravity_unit[:, 2],
    )
    score = 1.0 / (
      1.0
      + torch.square(tilt / max(UPRIGHT_TILT_SCALE_RAD, 1e-6))
    )
    return tilt, score

  rear_tilt_angle, rear_upright = _tilt_and_upright(rear_gravity_mid)
  front_tilt_angle, front_upright = _tilt_and_upright(front_gravity_mid)

  # Geometric mean means both halves must be reasonably upright; one excellent
  # half cannot completely hide one badly rolled half.
  upright = torch.sqrt(
    torch.clamp(rear_upright * front_upright, min=0.0, max=1.0)
  )

  lateral_velocity = torch.clamp(
    torch.square(midpoint_lin_vel_b[:, 1] / LATERAL_VELOCITY_SCALE_M_S),
    max=4.0,
  )
  vertical_velocity = torch.clamp(
    torch.square(midpoint_lin_vel_b[:, 2] / VERTICAL_VELOCITY_SCALE_M_S),
    max=4.0,
  )
  # Reconstruct approximate half-body angular rates around the virtual
  # midpoint. The relative axial rate is spine_qdot, split equally between the
  # two halves for this symmetric whole-body regularizer.
  spine_rate_vector = spine_qdot[:, None] * spine_axis_mid[None, :]
  rear_ang_mid = midpoint_ang_vel_b + 0.5 * spine_rate_vector
  front_ang_mid = midpoint_ang_vel_b - 0.5 * spine_rate_vector
  roll_pitch_rate = torch.clamp(
    0.5 * (
      torch.sum(
        torch.square(rear_ang_mid[:, :2] / ROLL_PITCH_RATE_SCALE_RAD_S),
        dim=1,
      )
      + torch.sum(
        torch.square(front_ang_mid[:, :2] / ROLL_PITCH_RATE_SCALE_RAD_S),
        dim=1,
      )
    ),
    max=4.0,
  ...
  zero_command_yaw_rate = torch.clamp(
    torch.square(
      midpoint_ang_vel_b[:, 2]
      / max(ZERO_COMMAND_YAW_RATE_SCALE_RAD_S, 1e-6)
    ),
    max=4.0,
  ) * (~yaw_active).float()

  # All nine actuators contribute to the general effort cost. The spine also
  # gets an explicit mechanical-power cost below so gratuitous twisting is not
  # free, while useful twist can still win through better tracking/stability.
  effort_scale = torch.full_like(actuator_force, XL330_SAFE_EFFORT_NM)
  effort_scale[:, spine_actuator_id] = ACTIVE_SPINE_SAFE_EFFORT_NM
  normalized_torque = torch.mean(
    torch.square(
      actuator_force / torch.clamp(effort_scale, min=1e-6)
    ),
    dim=1,
  )
  ...
  normalized_acceleration = torch.mean(
    torch.square(
      active_joint_acc / JOINT_ACCELERATION_SCALE_RAD_S2
    ),
    dim=1,
  )
  normalized_acceleration = torch.clamp(normalized_acceleration, max=4.0)
  ...
  motor_error = leg_joint_pos - leg_term._stand[None, :]
  ...
  leg_filter_correction = (
    leg_term.filter_correction
    / max(float(leg_term.cfg.action_scale_rad), 1e-6)
  )
  spine_filter_correction = (
    spine_term.filter_correction
    / max(float(spine_term.cfg.target_limit_rad), 1e-6)
  )
  filter_correction = torch.mean(
    torch.square(
      torch.cat(
        (leg_filter_correction, spine_filter_correction),
        dim=1,
      )
    ),
    dim=1,
  ...
  normalized_joint_offset = torch.abs(motor_error) / max(
    float(leg_term.cfg.joint_half_range_rad),
    1e-6,
  )
  ...
  # Locomotion does NOT penalize spine angle around zero. Only the last part of
  # the mechanical range is expensive, leaving most of the workspace free for
  # emergent coordination.
  spine_limit_excess = torch.clamp(
    torch.abs(spine_q) - ACTIVE_SPINE_LIMIT_FREE_RAD,
    min=0.0,
  )
  spine_limit_proximity = torch.clamp(
    torch.square(
      spine_limit_excess / max(ACTIVE_SPINE_LIMIT_SCALE_RAD, 1e-6)
    ),
    max=4.0,
  )

  spine_torque = actuator_force[:, spine_actuator_id]
  spine_power_w = torch.abs(spine_torque * spine_qdot)
  spine_power = torch.clamp(
    torch.square(
      spine_power_w / max(ACTIVE_SPINE_POWER_SCALE_W, 1e-6)
    ),
    max=4.0,
  )

  # Neutral spine is desirable only while truly standing. During locomotion,
  # q is free except for the near-limit and energy costs above.
  standing_spine_neutral = torch.clamp(
    torch.square(
      spine_q / max(ACTIVE_SPINE_STAND_SCALE_RAD, 1e-6)
    ),
    max=4.0,
  ) * standing.float()
  ...
  raw_terms = {
    ...
    "spine_limit_proximity": spine_limit_proximity,
    "spine_power": spine_power,
    "standing_spine_neutral": standing_spine_neutral,
    "spine_mean_bias": spine_mean_bias,
  }
  ...
  _safe_log(
    ...
    torch.mean(torch.abs(midpoint_ang_vel_b[:, 2])),
  )
  _safe_log(
    env,
    "Metrics/microtaur/yaw_error_abs_rad_s",
    torch.mean(torch.abs(command_yaw - midpoint_ang_vel_b[:, 2])),
  )
  _safe_log(
    env,
    "Metrics/microtaur/tilt_deg",
    torch.mean(torch.rad2deg(midpoint_tilt_angle)),
  )
  _safe_log(
    env,
    "Metrics/microtaur/rear_tilt_deg",
    torch.mean(torch.rad2deg(rear_tilt_angle)),
  )
  _safe_log(
    env,
    "Metrics/microtaur/front_tilt_deg",
    torch.mean(torch.rad2deg(front_tilt_angle)),
  )
  _safe_log(
    env,
    "Metrics/microtaur/relative_trunk_twist_deg",
    torch.mean(torch.abs(torch.rad2deg(spine_q))),
  )
  _safe_log(
    env,
    "Metrics/microtaur/spine_angle_abs_deg",
    torch.mean(torch.abs(torch.rad2deg(spine_q))),
  )
  _safe_log(
    env,
    "Metrics/microtaur/spine_mean_bias_deg",
    torch.mean(torch.rad2deg(state["spine_mean_q"])),
  )
  _safe_log(
    env,
    "Metrics/microtaur/spine_power_w",
    torch.mean(spine_power_w),
  )
  _safe_log(
    env,
...
# CPG-RL: replace the eight direct motor actions with four leg oscillators
# (cpg_action.py). Off by default so existing runs are unchanged.
...
def _configure_actions(
  ...
  stage_idx = PLAY_COMMAND_STAGE if play else 0
  stage_idx = max(0, min(stage_idx, len(COMMAND_STAGES) - 1))
  spine_scale = float(COMMAND_STAGES[stage_idx]["spine_action_scale_rad"])
  ...
  cfg.actions["spine_pos"] = MicrotaurActiveSpinePositionActionCfg(
    asset_cfg=SceneEntityCfg(
      "robot",
      joint_names=list(SPINE_JOINT_NAMES),
    ),
    scale=1.0,
    offset=0.0,
    preserve_order=True,
    use_default_offset=False,
    action_scale_rad=spine_scale,
    target_limit_rad=ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD,
    filter_horizon_s=ACTIVE_SPINE_FILTER_HORIZON_S,
    velocity_floor_rad_s=ACTIVE_SPINE_FILTER_VELOCITY_FLOOR_RAD_S,
    limit_margin_rad=ACTIVE_SPINE_FILTER_MARGIN_RAD,
    min_action_delay_steps=OLYMPUS_MIN_ACTION_DELAY_STEPS,
    max_action_delay_steps=OLYMPUS_MAX_ACTION_DELAY_STEPS,
  )
...
def _configure_observations(
  ...
  # Replace the inherited root-half kinematics with the virtual midpoint chassis
  # state. Keep the command observation untouched; it continues to provide
  # [vx, vy=0, yaw_rate] to the policy.
  for group_name in ("actor", "critic"):
    group = cfg.observations[group_name]

    base_lin_vel_term = group.terms.get("base_lin_vel")
    base_ang_vel_term = group.terms.get("base_ang_vel")
    projected_gravity_term = group.terms.get("projected_gravity")
    if base_lin_vel_term is None or base_ang_vel_term is None:
      raise KeyError(
        f"Observation group '{group_name}' is missing base velocity terms."
      )
    ...
    base_lin_vel_term.func = microtaur_midpoint_lin_vel
    base_lin_vel_term.params = {}
    base_ang_vel_term.func = microtaur_midpoint_ang_vel
  ...
  for group_name in ("actor", "critic"):
    group = cfg.observations[group_name]
    # Keep leg encoders separate from the explicit q/qdot spine observation so
    # the active spine is not duplicated.
  ...
  if CPG_ENABLED:
    from microtaur_velocity.cpg_action import cpg_phase_obs

    for group_name in ("actor", "critic"):
      cfg.observations[group_name].terms["cpg_phase"] = ObservationTermCfg(
        func=cpg_phase_obs,
...
def microtaur_terrain_level_tracking(
  env,
  env_ids: torch.Tensor | slice,
  command_name: str,
) -> dict[str, torch.Tensor]:
  """Log where robots sit in the terrain curriculum and why they move.

  Logging only. Must run before MJLab's terrain_levels term: that term moves the
  env origins of resetting robots, which would corrupt the walked distance. The
  promote/demote test mirrors mdp.terrain_levels_vel, so level fractions lag the
  update by one reset call.
  """
  terrain = env.scene.terrain
  generator = terrain.cfg.terrain_generator
  levels = terrain.terrain_levels
  stats = {
    f"frac_level_{level}": torch.mean((levels == level).float())
    for level in range(generator.num_rows)
  }

  robot = env.scene["robot"]
  distance = torch.norm(
    robot.data.root_link_pos_w[env_ids, :2] - env.scene.env_origins[env_ids, :2],
    dim=1,
  )
  command = env.command_manager.get_command(command_name)
  demote_below = (
  ...
  promoted = distance > generator.size[0] / 2
  demoted = (distance < demote_below) & ~promoted

  # The first reset happens at the spawn pose, where distance is meaningless.
  if distance.numel() == 0 or env.common_step_counter == 0:
    zero = torch.zeros((), device=env.device)
    stats.update(
      reset_distance_m=zero, demote_below_m=zero, frac_promoted=zero, frac_demoted=zero
    )
    return stats

  stats.update(
    reset_distance_m=torch.mean(distance),
    demote_below_m=torch.mean(demote_below),
    frac_promoted=torch.mean(promoted.float()),
    frac_demoted=torch.mean(demoted.float()),
  )
  return stats
  ...
  sensors.append(
    ContactSensorCfg(
      ...
      primary=ContactMatch(
        mode="geom",
        entity="robot",
        pattern=ACTIVE_BODY_COLLISION_GEOMS,
      ),
      secondary=ContactMatch(mode="body", pattern="terrain"),
      fields=("found", "force"),
      reduce="netforce",
...
def _configure_events(
  ...
  if "base_com" in cfg.events:
    cfg.events["base_com"].params["asset_cfg"] = SceneEntityCfg(
      "robot",
      body_names=[ACTIVE_SPINE_REAR_BODY, ACTIVE_SPINE_FRONT_BODY],
    )
...
def _make_base_cfg(
  ...
  if not HAS_ACTIVE_SPINE:
    raise RuntimeError(
      "This environment requires MICROTAUR_VARIANT=active_twist_microtaur."
    )
  if not CONTROL_SPINE:
    raise RuntimeError(
      "This environment requires MICROTAUR_CONTROL_SPINE=1 so the policy "
      "owns spine_joint_act."
    )
  ...
  _configure_actions(cfg, play=play)
  ...
  if rough:
    ...
    # Insert first so it sees env origins before terrain_levels moves them.
    cfg.curriculum = {
      "terrain_level_tracking": CurriculumTermCfg(
        func=microtaur_terrain_level_tracking,
