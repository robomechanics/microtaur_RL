"""Active-pitch Microtaur velocity-control environment.

This file keeps the rigid Microtaur leg controller and adds one actively
controlled pitch-spine degree of freedom. The eight leg actions retain the
predictive five-bar safety filter and 1-2 control-step delay used by the rigid
and other articulated variants.

The ninth policy action directly commands the active pitch spine. There is no
prescribed pitch trajectory and no positive reward for moving the spine: during
locomotion the policy is free to use any pitch angle that improves commanded
forward/yaw velocity tracking enough to justify actuator effort and near-limit
costs. Spine authority is expanded gradually by curriculum while the physical
joint remains hard-clamped to +/-30 degrees.

Forward/yaw tracking and body-state observations use a virtual midpoint chassis
frame formed from the two trunk halves. This prevents either half from becoming
the privileged body reference while the policy articulates the pitch joint.
"""

from __future__ import annotations

ENV_CFG_REVISION = "2026-09-01-active-pitch-final-xml-v2"
...
# ROBOTIS XL330-M077-T output-shaft data for the eight LEG motors:
...
# ROBOTIS XL330-M288-T output-shaft data for the PITCH-SPINE motor:
# supply voltage: (stall torque [N m], no-load speed [rpm], stall current [A])
_XL330_M288_TABLE = {
  3.7: (0.42, 76.0, 1.11),
  5.0: (0.52, 103.0, 1.47),
...
if (
  XL330_SUPPLY_VOLTAGE not in _XL330_M077_TABLE
  or XL330_SUPPLY_VOLTAGE not in _XL330_M288_TABLE
):
  raise ValueError(
...
# Legs remain XL330-M077-T. Keep the existing XL330_* names for the leg model
# so the rest of this environment is unchanged.
(
  XL330_STALL_TORQUE_NM,
  XL330_NO_LOAD_SPEED_RPM,
  XL330_STALL_CURRENT_A,
) = _XL330_M077_TABLE[XL330_SUPPLY_VOLTAGE]
XL330_NO_LOAD_SPEED_RAD_S = XL330_NO_LOAD_SPEED_RPM * 2.0 * math.pi / 60.0

# Spine is XL330-M288-T.
(
  XL330_M288_STALL_TORQUE_NM,
  XL330_M288_NO_LOAD_SPEED_RPM,
  XL330_M288_STALL_CURRENT_A,
) = _XL330_M288_TABLE[XL330_SUPPLY_VOLTAGE]
XL330_M288_NO_LOAD_SPEED_RAD_S = (
  XL330_M288_NO_LOAD_SPEED_RPM * 2.0 * math.pi / 60.0
)
...
# Spine uses the same training safety fractions, applied to the M288-T limits.
XL330_M288_SAFE_EFFORT_NM = (
  XL330_SAFE_TORQUE_FRACTION * XL330_M288_STALL_TORQUE_NM
)
XL330_M288_VELOCITY_LIMIT_RAD_S = (
  XL330_VELOCITY_LIMIT_FRACTION * XL330_M288_NO_LOAD_SPEED_RAD_S
)
...
# Active pitch spine. The final CAD XML uses the battery/top half as the
# floating front root and the bearing-named Arduino/rear half as the child of
# spine_joint_act.
ACTIVE_SPINE_JOINT_NAME = "spine_joint_act"
ACTIVE_PITCH_FRONT_BODY = "pitch_microtaur_top_half_1"
ACTIVE_PITCH_REAR_BODY = "4390n173_flanged_ultra_thin_ball_bearing_2__15"
ACTIVE_BODY_COLLISION_GEOMS = ("front_body_collision", "rear_body_collision")

# In the raw CAD export the child frame quaternion maps the local +Z hinge axis
# onto the robot lateral +Y axis at the flat configuration. This is therefore a
# true pitch articulation.
ACTIVE_SPINE_AXIS_ROOT = (0.0, 1.0, 0.0)

# Final pitch XML geometry at spine q=0, expressed in the battery/front root
# frame.  These are the midpoints of each physical A/E motor pair using the
# canonical numbering leg1=BR, leg2=BL, leg3=FL, leg4=FR.
PITCH_HIP_MIDPOINTS_ROOT_M = np.asarray(
  [
    [-0.06930121, -0.06750000, 0.01150000],  # leg1 BR
    [-0.06930121, +0.01842000, 0.01150000],  # leg2 BL
    [+0.10374890, +0.01912520, 0.01150000],  # leg3 FL
    [+0.10374890, -0.06650000, 0.01150000],  # leg4 FR
  ],
  dtype=np.float64,
)

# The final XML reverses the A/E hinge axes on mirrored legs 2 and 3 so the
# existing rigid stand commands produce the same physical leg motion on all
# four legs.  Their passive B/D hinge axes remain unflipped.  Therefore the
# rigid analytical IK's passive q values must be converted before they are
# written to the pitch XML: legs 2/3 B and D change sign; A/E do not.
PITCH_XML_PASSIVE_Q_SIGNS = (1.0, -1.0, -1.0, 1.0)

# Physical hard stop requested by the morphology: +/-30 deg from flat. Policy
# targets may use the full +/-30 degree mechanical range; the last five degrees
# are increasingly expensive through the limit cost and the predictive filter
# prevents commands from driving farther outward at the stop.
ACTIVE_SPINE_HARD_LIMIT_RAD = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_HARD_LIMIT_RAD",
    str(math.radians(30.0)),
  )
)
ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD",
    str(math.radians(30.0)),
  )
)
ACTIVE_SPINE_LIMIT_FREE_RAD = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_LIMIT_FREE_RAD",
    str(math.radians(25.0)),
  )
)
ACTIVE_SPINE_LIMIT_SCALE_RAD = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_LIMIT_SCALE_RAD",
    str(math.radians(5.0)),
  )
)
ACTIVE_SPINE_STAND_SCALE_RAD = float(
  os.environ.get(
    "MICROTAUR_ACTIVE_SPINE_STAND_SCALE_RAD",
...
MICROTAUR_JOINT_HALF_RANGE_RAD = float(
  os.environ.get("MICROTAUR_JOINT_HALF_RANGE_RAD", "0.95")
)
...
def _make_xml_validated_spec() -> mujoco.MjSpec:
  """Load and validate the final active-pitch XML coordinate convention."""
  spec = get_spec()

  # q=0 must be the raw symmetric CAD pose: no joint reference offsets.
  for joint_name in (*LEG_JOINT_NAMES, *PASSIVE_JOINT_NAMES, ACTIVE_SPINE_JOINT_NAME):
    joint = spec.joint(joint_name)
    if joint is None:
      raise ValueError(f"Missing required pitch joint: {joint_name}")
    if abs(float(joint.ref)) > 1.0e-10:
      raise ValueError(
        f"Pitch joint {joint_name!r} has ref={float(joint.ref):.6g}; "
        "the final XML requires zero joint refs."
      )

  # Final XML actuator-axis convention: mirrored legs 2/3 are reversed in XML,
  # while legs 1/4 retain +Z.  Do not duplicate this sign correction in Python.
  expected_axis_z = {
    "leg1_a_joint_act": +1.0, "leg1_e_joint_act": +1.0,
    "leg2_a_joint_act": -1.0, "leg2_e_joint_act": -1.0,
    "leg3_a_joint_act": -1.0, "leg3_e_joint_act": -1.0,
    "leg4_a_joint_act": +1.0, "leg4_e_joint_act": +1.0,
  }
  for joint_name, expected_z in expected_axis_z.items():
    axis = np.asarray(spec.joint(joint_name).axis, dtype=np.float64)
    expected = np.asarray((0.0, 0.0, expected_z), dtype=np.float64)
    if not np.allclose(axis, expected, atol=1.0e-8):
      raise ValueError(
        f"Pitch XML axis mismatch for {joint_name}: got {axis}, "
        f"expected {expected}."
      )
  ...
  if spine_joint is None:
    raise ValueError(
      "Active-pitch environment requires joint 'spine_joint_act'."
    )
  ...
  for geom_name in ACTIVE_BODY_COLLISION_GEOMS:
    if spec.geom(geom_name) is None:
      raise ValueError(f"Missing active-pitch body collision geom: {geom_name}")
...
COMMAND_STAGES = (
  {
    ...
    "ang_vel_z": (0.00, 0.00),
    "standing": 0.00,
    "spine_action_scale_rad": math.radians(8.0),
  },
  {
    "step": 25_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.10, 0.10),
    "standing": 0.05,
    "spine_action_scale_rad": math.radians(12.0),
  },
  {
    "step": 50_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.15, 0.15),
    "standing": 0.05,
    "spine_action_scale_rad": math.radians(18.0),
  },
  {
    "step": 70_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.20, 0.20),
    "standing": 0.10,
    "spine_action_scale_rad": math.radians(24.0),
  },
  {
    "step": 90_000,
    "lin_vel_x": (0.08, 0.20),
    "ang_vel_z": (-0.25, 0.25),
    "standing": 0.10,
    "spine_action_scale_rad": math.radians(30.0),
  },
)
...
MICROTAUR_NOMINAL_ROOT_HEIGHT_M = float(
  FOOT_SPHERE_RADIUS_M
  - np.min(PITCH_HIP_MIDPOINTS_ROOT_M[:, 2] + _IK_NOMINAL_FOOT_XZ[:, 1])
)
...
def microtaur_ik_consistent_reset(
  ...
  *,
  ...
  # Convert the shared rigid IK coordinates to the final pitch XML's joint
  # coordinates.  A/E are already correct because their axis reversal is in
  # the XML itself.  Only passive B/D on mirrored legs 2/3 need a q sign flip.
  passive_sign = torch.tensor(
    PITCH_XML_PASSIVE_Q_SIGNS,
    device=device,
    dtype=dtype,
  )[None, :]
  full_q = full_q.clone()
  full_q[:, :, 1] = full_q[:, :, 1] * passive_sign
  full_q[:, :, 3] = full_q[:, :, 3] * passive_sign
  ...
  # The leg IK is defined with a flat/neutral pitch spine. Reset the active spine exactly
...
MICROTAUR_REWARD_SCALES = {
  ...
  "body_height": 1.00,
  "upright": 0.75,
  "feet_air_time": 0.35, #was 0.15
  "moving_contact_pattern": 0.25, #was 0.20
  "standing_pose": 0.50,
  "lateral_velocity": -0.15,
  "vertical_velocity": -0.10,
  "roll_pitch_rate": -0.10,
  "normalized_torque": -0.03,
  "normalized_acceleration": -0.025, #was -0.01
  "action_rate": -0.07, #was -0.04
  "undesired_contacts": -2.00,
  "filter_correction": -0.10,
  "joint_limit_proximity": -0.05,

  # No positive spine-use reward: the spine must help the locomotion task.
...
YAW_COMMAND_ACTIVE_THRESHOLD_RAD_S = float(
  os.environ.get("MICROTAUR_YAW_ACTIVE_THRESHOLD_RAD_S", "0.03")
)
...
class MicrotaurActiveSpinePositionActionCfg(JointPositionActionCfg):
  """One-dimensional active pitch target with limit filtering and delay."""

  class_type: type = MISSING

  action_scale_rad: float = math.radians(8.0)
...
class MicrotaurActiveSpinePositionAction(JointPositionAction):
  ...
  def __init__(self, cfg: MicrotaurActiveSpinePositionActionCfg, env):
    ...
    delay_size = max(1, int(cfg.max_action_delay_steps) + 1)
    self._delay_buffer = torch.zeros((env.num_envs, delay_size, 1), device=device)
    self._delay_steps = torch.full(
      (env.num_envs,),
      int(cfg.min_action_delay_steps),
      device=device,
  ...
  def _reset_delay_state(self, reset: torch.Tensor, current_position: torch.Tensor) -> None:
    if not torch.any(reset):
      return
    count = int(torch.sum(reset).item())
    self._delay_steps[reset] = torch.randint(
  ...
  def _filter_target(
    ...
    limit = min(float(self.cfg.target_limit_rad), ACTIVE_SPINE_HARD_LIMIT_RAD - 1e-4)
    lower = -limit
    upper = limit
  ...
  def process_actions(self, actions: torch.Tensor):
    ...
    action_scale = min(float(self.cfg.action_scale_rad), float(self.cfg.target_limit_rad))
...
def _microtaur_active_spine_indices(env) -> tuple[int, int, int, int]:
  """Return local IDs for spine joint, front trunk, rear trunk, spine actuator."""
  ...
  if len(joint_ids) != 1 or tuple(joint_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active pitch spine joint could not be resolved exactly once.")

  front_ids, front_names = robot.find_bodies(
    [ACTIVE_PITCH_FRONT_BODY],
    preserve_order=True,
  )
  rear_ids, rear_names = robot.find_bodies(
    [ACTIVE_PITCH_REAR_BODY],
    preserve_order=True,
  )
  if len(front_ids) != 1 or tuple(front_names) != (ACTIVE_PITCH_FRONT_BODY,):
    raise RuntimeError("Active-pitch front/root trunk body could not be resolved.")
  if len(rear_ids) != 1 or tuple(rear_names) != (ACTIVE_PITCH_REAR_BODY,):
    raise RuntimeError("Active-pitch rear/child trunk body could not be resolved.")
  ...
  if len(actuator_ids) != 1 or tuple(actuator_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active pitch spine actuator could not be resolved exactly once.")
...
def microtaur_spine_joint_position(env) -> torch.Tensor:
  """Active pitch-spine angle q [rad]."""
  robot = env.scene["robot"]
...
def microtaur_spine_joint_velocity(env) -> torch.Tensor:
  """Active pitch-spine rate qdot [rad/s]."""
  robot = env.scene["robot"]
  spine_joint_id, _, _, _ = _microtaur_active_spine_indices(env)
...
def _microtaur_midpoint_frame_state(
  ...
  """Return virtual-midpoint linear velocity, angular velocity, and gravity.

  Front/root and rear/child trunk world velocities are averaged first. The mean
  is expressed in the front/root frame and rotated by -q/2 about the physical
  pitch-spine axis, yielding the halfway chassis frame.

  This prevents one trunk half from becoming the rewarded body while the other
  moves through the pitch articulation.
  """
  robot = env.scene["robot"]
  spine_joint_id, front_body_id, rear_body_id, _ = (
    _microtaur_active_spine_indices(env)
  )
  ...
  mean_lin_w = 0.5 * (front_lin_w + rear_lin_w)
  mean_ang_w = 0.5 * (front_ang_w + rear_ang_w)

  front_quat_w = robot.data.body_link_quat_w[:, front_body_id]
  mean_lin_front = quat_apply_inverse(front_quat_w, mean_lin_w)
  mean_ang_front = quat_apply_inverse(front_quat_w, mean_ang_w)

  spine_q = robot.data.joint_pos[:, spine_joint_id]
  axis = torch.tensor(
    ACTIVE_SPINE_AXIS_ROOT,
    device=mean_lin_front.device,
    dtype=mean_lin_front.dtype,
  )
  half_angle = -0.5 * spine_q

  midpoint_lin_b = _rotate_vector_about_axis(
    mean_lin_front,
    axis,
    half_angle,
  )
  midpoint_ang_b = _rotate_vector_about_axis(
    mean_ang_front,
    axis,
    half_angle,
  )
...
def microtaur_scaled_walk_reward(
  ...
  """Rigid-baseline velocity reward plus emergent active-pitch spine costs."""
  robot = env.scene["robot"]
  ...
  actual_yaw = midpoint_ang_vel_b[:, 2]

  translation_active = torch.abs(command_x) > 0.03
  yaw_active = torch.abs(command_yaw) > YAW_COMMAND_ACTIVE_THRESHOLD_RAD_S
  ...
  tilt_angle = torch.atan2(
    torch.linalg.vector_norm(gravity_norm[:, :2], dim=1),
    -gravity_norm[:, 2],
  )
  upright = 1.0 / (
    1.0
  ...
  lateral_velocity = torch.clamp(
    torch.square(
      midpoint_lin_vel_b[:, 1] / LATERAL_VELOCITY_SCALE_M_S
    ),
    max=4.0,
  )
  vertical_velocity = torch.clamp(
    torch.square(
      midpoint_lin_vel_b[:, 2] / VERTICAL_VELOCITY_SCALE_M_S
    ),
    max=4.0,
  )
  roll_pitch_rate = torch.clamp(
    torch.sum(
      torch.square(
        midpoint_ang_vel_b[:, :2] / ROLL_PITCH_RATE_SCALE_RAD_S
      ),
      dim=1,
    ),
    max=4.0,
  ...
  # Only true standing prefers a neutral pitch spine. During locomotion,
  # including straight walking, the pitch angle is free to emerge.
  ...
  _safe_log(
    ...
    torch.mean(torch.abs(actual_yaw)),
  )
  _safe_log(
    env,
    "Metrics/microtaur/yaw_error_abs_rad_s",
    torch.mean(torch.abs(command_yaw - actual_yaw)),
  )
  _safe_log(
    env,
  ...
  _safe_log(
    env,
    "Metrics/microtaur/spine_requested_deg",
    torch.mean(torch.rad2deg(spine_term.requested_targets.squeeze(-1))),
  )
  _safe_log(
    env,
    "Metrics/microtaur/spine_applied_deg",
    torch.mean(torch.rad2deg(spine_term.applied_targets.squeeze(-1))),
  )
  _safe_log(
    env,
...
def _configure_actions(
  ...
  # Eight leg actions remain identical to the working rigid/spined baseline.
  ...
  # Ninth action: direct active-pitch target. No command bias or prescribed
  # pitching pattern; normalized [-1,1] maps to the current curriculum range.
  cfg.actions["spine_pos"] = MicrotaurActiveSpinePositionActionCfg(
    asset_cfg=SceneEntityCfg("robot", joint_names=[ACTIVE_SPINE_JOINT_NAME]),
    scale=1.0,
    offset=0.0,
    preserve_order=True,
    use_default_offset=False,
...
def _configure_sensors(
  ...
  if rough:
    sensors.append(
      make_microtaur_terrain_scan_cfg(
        root_body_name=ACTIVE_PITCH_FRONT_BODY,
        debug_vis=play,
      )
    )
...
def _configure_events(
  ...
  if "base_com" in cfg.events:
    cfg.events["base_com"].params["asset_cfg"] = SceneEntityCfg(
      "robot",
      body_names=[ACTIVE_PITCH_FRONT_BODY, ACTIVE_PITCH_REAR_BODY],
    )
...
def _make_base_cfg(
  ...
  # The final CAD XML intentionally keeps its exported MuJoCo model name
  # ``pitch_microtaur`` even when it lives in the active_pitch_microtaur
  # morphology folder.  Accept the older normalized name as well.
  if MODEL_NAME not in {"pitch_microtaur", "active_pitch_microtaur"}:
    raise RuntimeError(
      "This env_cfgs.py requires the active pitch Microtaur XML; "
      f"loaded model is {MODEL_NAME!r}."
    )
  ...
  cfg.viewer.body_name = ACTIVE_PITCH_FRONT_BODY
...
def microtaur_velocity_pitch_flat_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  return _make_base_cfg(play=play, rough=False)


def microtaur_velocity_pitch_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
...
# ---------------------------------------------------------------------------
# Public task-registration compatibility
# ---------------------------------------------------------------------------
# microtaur_velocity/__init__.py historically imports these "yaw" factory
# names for the registered flat/rough tasks. Keep that module API stable
# even though this file now implements the active-pitch morphology.
def microtaur_velocity_yaw_flat_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  return microtaur_velocity_pitch_flat_env_cfg(play=play)


def microtaur_velocity_yaw_rough_env_cfg(
  play: bool = False,
) -> ManagerBasedRlEnvCfg:
  return microtaur_velocity_pitch_rough_env_cfg(play=play)
