"""Active-yaw Microtaur velocity-control environment.

This file is the rigid Microtaur velocity environment plus one actively
controlled yaw-spine degree of freedom. The eight leg actions keep the same
predictive five-bar safety filter and 1-2 control-step delay as the rigid robot.

The ninth policy action is a learned residual around a command-proportional
spine steering bias:

  q_spine_target = sign * gain * yaw_command + residual_scale * action_spine

The target is clamped inside the physical spine limits and passed through its
own delayed position-action filter. This gives the robot an immediately useful
steering prior while preserving policy authority over how much/when to bend.

Forward/yaw tracking and base observations use a virtual midpoint chassis frame
formed from the front/rear trunk world velocities. The spine is not rewarded
for moving. It pays only effort/power, near-limit, action-rate, and straight-
command neutral costs, so deviations from the proportional bias must earn their
keep through better locomotion.
"""

from __future__ import annotations

ENV_CFG_REVISION = "2026-08-30-active-yaw-spine-residual-v1"
...
# Active yaw spine. The current active-yaw XML has the battery/front half as
# the floating root and the Arduino/rear half as the child of spine_joint_act.
ACTIVE_SPINE_JOINT_NAME = "spine_joint_act"
ACTIVE_YAW_FRONT_BODY = "s_microtaur_bottom_half_motor_mount"
ACTIVE_YAW_REAR_BODY = "s_microtaur_top_half_motor_mount"
ACTIVE_BODY_COLLISION_GEOMS = ("front_body_collision", "rear_body_collision")

# The rear child has a 180 degree neutral rotation about Y, so its local +Z
# hinge axis maps to -Z in the front/root chassis frame.
ACTIVE_SPINE_AXIS_ROOT = (0.0, 0.0, -1.0)
...
ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD = float(
  os.environ.get("MICROTAUR_ACTIVE_SPINE_SAFE_TARGET_LIMIT_RAD", "0.30")
)
...
# q_bias = sign * gain_s * yaw_command.
# Defaults: |wz|=0.25 rad/s -> |q_bias|=0.20 rad ~= 11.5 degrees.
# The policy can add a +/-0.10 rad residual before the final +/-0.30 rad clamp.
ACTIVE_SPINE_STEERING_GAIN_S = float(
  os.environ.get("MICROTAUR_YAW_SPINE_STEERING_GAIN_S", "0.80")
)
ACTIVE_SPINE_STEERING_SIGN = float(
  os.environ.get("MICROTAUR_YAW_SPINE_STEERING_SIGN", "1.0") # was -1.0
)
ACTIVE_SPINE_RESIDUAL_SCALE_RAD = float(
  os.environ.get("MICROTAUR_YAW_SPINE_RESIDUAL_SCALE_RAD", "0.10")
)
ACTIVE_SPINE_ZERO_YAW_NEUTRAL_SCALE_RAD = float(
  os.environ.get("MICROTAUR_YAW_SPINE_NEUTRAL_SCALE_RAD", "0.10")
)
...
def _make_xml_validated_spec() -> mujoco.MjSpec:
  """Load the active-yaw XML and repair RL-critical CAD defaults."""
  spec = get_spec()
  ...
  if spine_joint is None:
    raise ValueError(
      "Active-yaw environment requires joint 'spine_joint_act'."
    )
  ...
  for geom_name in ACTIVE_BODY_COLLISION_GEOMS:
    if spec.geom(geom_name) is None:
      raise ValueError(f"Missing active-yaw body collision geom: {geom_name}")
...
def microtaur_ik_consistent_reset(
  ...
  *,
  ...
  # The leg IK is defined at zero yaw. Reset the active spine exactly
...
MICROTAUR_REWARD_SCALES = {
  ...
  "upright": 0.75,
  "feet_air_time": 0.15,
  "moving_contact_pattern": 0.20,
  "standing_pose": 0.50,
  "lateral_velocity": -0.15,
  "vertical_velocity": -0.15, #was -0.10
  "roll_pitch_rate": -0.10,
  "normalized_torque": -0.03,
  "normalized_acceleration": -0.01,
  "action_rate": -0.04,
...
class MicrotaurActiveSpinePositionActionCfg(JointPositionActionCfg):
  """Command-biased one-DOF yaw-spine action with a learned residual."""

  class_type: type = MISSING

  command_name: str = "twist"
  steering_gain_s: float = ACTIVE_SPINE_STEERING_GAIN_S
  steering_sign: float = ACTIVE_SPINE_STEERING_SIGN
  residual_scale_rad: float = ACTIVE_SPINE_RESIDUAL_SCALE_RAD
  ...
  def __init__(
    ...
    **kwargs,
    ...
    self.steering_gain_s = float(steering_gain_s)
    self.steering_sign = float(steering_sign)
    self.residual_scale_rad = float(residual_scale_rad)
...
class MicrotaurActiveSpinePositionAction(JointPositionAction):
  """Yaw spine target = proportional command bias + learned residual."""
  ...
  def __init__(self, cfg: MicrotaurActiveSpinePositionActionCfg, env):
    ...
    self._bias_targets = torch.zeros((env.num_envs, 1), device=device)
    self._residual_targets = torch.zeros((env.num_envs, 1), device=device)
  ...
  @property
  def bias_targets(self) -> torch.Tensor:
    return self._bias_targets

  @property
  def residual_targets(self) -> torch.Tensor:
    return self._residual_targets
  ...
  def _reset_delay_state(
    ...
    self._bias_targets[reset] = 0.0
    self._residual_targets[reset] = 0.0
    self._requested_targets[reset] = reset_position
    self._safe_targets[reset] = reset_position
    self._applied_targets[reset] = reset_position
    self._filter_correction[reset] = 0.0
  ...
  def process_actions(self, actions: torch.Tensor):
    ...
    command = self._walk_env.command_manager.get_command(self.cfg.command_name)
    if command is None or command.ndim != 2 or command.shape[1] < 3:
      raise RuntimeError(
        f"Expected command '{self.cfg.command_name}' with shape [N, >=3]."
      )

    command_yaw = command[:, 2:3]
    steering_command = torch.where(
      torch.abs(command_yaw) > YAW_COMMAND_ACTIVE_THRESHOLD_RAD_S,
      command_yaw,
      torch.zeros_like(command_yaw),
    )
    bias = (
      float(self.cfg.steering_sign)
      * float(self.cfg.steering_gain_s)
      * steering_command
    )
    residual = float(self.cfg.residual_scale_rad) * actions

    limit = min(
      float(self.cfg.target_limit_rad),
      ACTIVE_SPINE_HARD_LIMIT_RAD - 1e-4,
    )
    requested = torch.clamp(bias + residual, min=-limit, max=limit)
    safe, blend = self._filter_target(position, velocity, requested)

    self._bias_targets = bias
    self._residual_targets = residual
    self._requested_targets = requested
    self._safe_targets = safe
    self._filter_correction = requested - safe
    self._filter_blend = blend
...
def _microtaur_active_spine_indices(env) -> tuple[int, int, int, int]:
  ...
  if len(joint_ids) != 1 or tuple(joint_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active yaw spine joint could not be resolved exactly once.")

  front_ids, front_names = robot.find_bodies(
    [ACTIVE_YAW_FRONT_BODY],
    preserve_order=True,
  )
  rear_ids, rear_names = robot.find_bodies(
    [ACTIVE_YAW_REAR_BODY],
    preserve_order=True,
  )
  if len(front_ids) != 1 or tuple(front_names) != (ACTIVE_YAW_FRONT_BODY,):
    raise RuntimeError("Active-yaw front/root trunk body could not be resolved.")
  if len(rear_ids) != 1 or tuple(rear_names) != (ACTIVE_YAW_REAR_BODY,):
    raise RuntimeError("Active-yaw rear/child trunk body could not be resolved.")
  ...
  if len(actuator_ids) != 1 or tuple(actuator_names) != (ACTIVE_SPINE_JOINT_NAME,):
    raise RuntimeError("Active yaw spine actuator could not be resolved exactly once.")
...
def microtaur_spine_joint_velocity(env) -> torch.Tensor:
  """Active yaw-spine rate qdot [rad/s]."""
  robot = env.scene["robot"]
  spine_joint_id, _, _, _ = _microtaur_active_spine_indices(env)
...
def _microtaur_midpoint_frame_state(
  ...
  yaw-spine axis, yielding the halfway chassis frame.

  This prevents yaw-reward hacking where only one trunk half follows the
  command while the other counter-rotates through the spine.
  """
  robot = env.scene["robot"]
...
def microtaur_scaled_walk_reward(
  ...
  """Rigid-baseline velocity reward plus the active yaw-spine costs."""
  robot = env.scene["robot"]
  ...
  if action.shape[1] != expected_action_dim:
    raise RuntimeError(
      f"Active-yaw policy must expose {expected_action_dim} actions; "
      f"got {action.shape[1]}."
    )
  if actuator_force.shape[1] != expected_actuator_dim:
    raise RuntimeError(
      f"Active-yaw robot must expose {expected_actuator_dim} actuators; "
  ...
  # Straight command -> weak preference for a straight chassis.
  # During turns there is deliberately no q->0 penalty.
  zero_yaw_spine_neutral = torch.clamp(
    torch.square(
      spine_q
      / max(ACTIVE_SPINE_ZERO_YAW_NEUTRAL_SCALE_RAD, 1e-6)
    ),
    max=4.0,
  ) * (~yaw_active).float()
  ...
  raw_terms = {
    ...
    "zero_yaw_spine_neutral": zero_yaw_spine_neutral,
  }
  ...
  _safe_log(
    env,
    "Metrics/microtaur/spine_bias_target_deg",
    torch.mean(torch.rad2deg(spine_term.bias_targets.squeeze(-1))),
  )
  _safe_log(
    env,
    "Metrics/microtaur/spine_residual_deg",
    torch.mean(torch.rad2deg(spine_term.residual_targets.squeeze(-1))),
  )
  _safe_log(
    env,
...
def _configure_actions(
  ...
  # Eight leg actions stay identical to the rigid baseline.
  ...
  # Ninth action: residual freedom around the proportional yaw-spine bias.
  cfg.actions["spine_pos"] = MicrotaurActiveSpinePositionActionCfg(
    asset_cfg=SceneEntityCfg(
      "robot",
      joint_names=[ACTIVE_SPINE_JOINT_NAME],
    ),
    scale=1.0,
    offset=0.0,
    preserve_order=True,
    use_default_offset=False,
    command_name="twist",
    steering_gain_s=ACTIVE_SPINE_STEERING_GAIN_S,
    steering_sign=ACTIVE_SPINE_STEERING_SIGN,
    residual_scale_rad=ACTIVE_SPINE_RESIDUAL_SCALE_RAD,
...
def _configure_sensors(
  ...
  if rough:
    sensors.append(
      make_microtaur_terrain_scan_cfg(
        root_body_name=ACTIVE_YAW_FRONT_BODY,
        debug_vis=play,
      )
    )
...
def _configure_events(
  ...
  if "base_com" in cfg.events:
    cfg.events["base_com"].params["asset_cfg"] = SceneEntityCfg(
      "robot",
      body_names=[ACTIVE_YAW_FRONT_BODY, ACTIVE_YAW_REAR_BODY],
    )
...
def _make_base_cfg(
  ...
  if MODEL_NAME != "active_yaw_microtaur":
    raise RuntimeError(
      "This env_cfgs.py requires MICROTAUR_VARIANT=active_yaw_microtaur; "
  ...
  cfg.viewer.body_name = ACTIVE_YAW_FRONT_BODY
