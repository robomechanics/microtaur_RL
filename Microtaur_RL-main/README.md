# mjlab example: microtaur_velocity
...
src/microtaur_velocity/
  ...
  microtaur_velocity/
    microtaur_velocity_constants.py            # Robot definition (actuators, collision, init state)
    xmls/
...
microtaur_velocity = "microtaur_velocity"
```
...
`microtaur_velocity_constants.py` provides the robot's `EntityCfg`: a MuJoCo spec
...
uv run play Mjlab-Velocity-Flat-microtaur_velocity --agent zero

# Train
CUDA_VISIBLE_DEVICES=0 uv run train Mjlab-Velocity-Flat-microtaur_velocity \
  --env.scene.num-envs 4096 \
  --agent.max-iterations 3_000

# Play the trained checkpoint
uv run play Mjlab-Velocity-Flat-microtaur_velocity --wandb-run-path <wandb-run-path>
```
